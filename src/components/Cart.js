    ...
    <div className="p-8">
      ...
      <div className="space-y-4">
        {cartItems.map((item) => (
          <div
            key={item.id}
            className="flex sm:flex-row flex-col items-center justify-between border p-4 rounded-lg shadow"
          >
            ...
            <button onClick={() => removeFromCart(item.id)} className="hover:bg-red-600 px-3 py-1 rounded-md font-bold hover:text-white">Del</button>
      ...
      <div className="mt-6 text-right">
        ...
        <Link to="/checkout">
          <button className="mt-4 px-6 py-2 bg-red-600 text-white font-bold rounded hover:bg-green-500">
            Proceed to Checkout
          </button>
        </Link>
      </div>
